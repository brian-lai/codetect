...
cmd_help() {
    ...
    echo ""
    echo "Commands:"
    echo "  init [-f|--force]                  Create/merge .mcp.json in current directory"
    echo "  index [flags] [path]               Index symbols and chunks (default: current directory)"
    echo "    -f, --force                        Force full reindex (default: incremental)"
    echo "    -v, --verbose                      Enable verbose output"
    echo "        --json                         Output results as JSON"
    echo "        --v1                           Use legacy ctags indexer (deprecated)"
    echo "  embed [flags] [path]               Generate embeddings for semantic search"
    echo "    -f, --force                        Re-embed all chunks (ignore cache)"
    echo "    -j, --parallel <N>                 Parallel workers (default: 10)"
    echo "        --provider <ollama|litellm>    Override embedding provider"
    echo "        --model <name>                 Override embedding model"
    echo "  stats [--json] [path]              Show index statistics"
    echo "  doctor                             Check installation and dependencies"
    echo "  reconfigure                        Interactive config wizard (database, embedding, model)"
    echo "  migrate [--dry-run|-n] [paths...]  Discover existing indexes and register them"
    echo "  mcp                                Start MCP server (used by .mcp.json)"
    echo "  daemon <command>                   Manage background indexing daemon"
    echo "  registry <command>                 Manage project registry"
    echo "  update [flags]                     Update to latest version from GitHub"
    echo "    -f, --force                        Rebuild even if already on latest"
    echo "        --pre                          Include pre-release versions"
    echo "    -v, --version <tag>                Install specific version"
    echo "  version                            Show installed version"
    echo "  help                               Show this help message"
    echo ""
    echo "Daemon Commands:"
    echo "  daemon start       Start background daemon"
    echo "  daemon stop        Stop daemon gracefully"
    echo "  daemon status      Show daemon status"
    echo "  daemon logs [N]    View last N log lines (default: 50)"
    echo ""
    echo "Registry Commands:"
    echo "  registry list          List registered projects"
    echo "  registry add [path]    Add project (default: current directory)"
    echo "  registry remove <path> Remove a project"
    echo "  registry stats         Show aggregate stats across all projects"
    echo ""
    echo "Configuration:"
